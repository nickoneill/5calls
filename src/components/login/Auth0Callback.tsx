...
import { AuthResponse, Auth0Config } from '../shared/authModels';
...
export class Auth0Callback extends React.Component<
  Auth0CallbackProps,
  Auth0CallbackState
> {
  ...
  componentDidMount() {
    this.loginService.handleAuthentication().then(response => {
      this.props.handleAuthentication(response).then(_ => {
        window.setTimeout(() => {
  ...
  render() {
    if (this.state.doneRedirect) {
      const redirect = this.props.redirectPath ? this.props.redirectPath : '/';
